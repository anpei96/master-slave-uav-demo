...
def imagewarping(img1, img2, H, min_x, max_x, min_y, max_y):
    ...
    mask = np.zeros_like(warped_mask2)
    row_indices, col_indices = np.where(warped_mask2 == 1)
    for row, col in zip(row_indices, col_indices):
        mask[row, col] = 255
    mask = mask[off[1]:off[1] + img1.shape[0], off[0]:off[0] + img1.shape[1]]
    mask = mask.astype(np.uint8)
    #warped_img8 = histogram.Histogram_Matching(warped_img6,warped_img7,mask)
    """
    return warped_img6

def imagewarping1(img1, img2, H, min_x, max_x, min_y, max_y):
    off = [int(round(-min_x)), int(round(-min_y))]
    map_x, map_y = transform(H, max_x, min_x, max_y, min_y)
    warped_img2 = cv.remap(img2, map_x, map_y, interpolation=cv.INTER_LINEAR, borderMode=cv.BORDER_REFLECT)
    mask2 = np.ones((img2.shape[0], img2.shape[1]))
    ...
    warped_img6 = warped_img3[off[1]:off[1] + img1.shape[0], off[0]:off[0] + img1.shape[1]]
    warped_img7 = warped_img5[off[1]:off[1] + img1.shape[0], off[0]:off[0] + img1.shape[1]]
    result = cv.addWeighted(warped_img6, 0.7, warped_img7, 0.3, 0)
    return warped_img6, result, warped_img7

def imagewarping2(img1, img2, H, min_x, max_x, min_y, max_y, C1 = None, C2 =None):
    ...
    for c in range(3):
        warped_img3[:, :, c] = warped_img2[:, :, c] * warped_mask2
    warped_img4 = warped_img3[off[1]:off[1] + img1.shape[0], off[0]:off[0] + img1.shape[1]]
    return warped_img4
